...
## 5-1. Suno로 만들 수 없는 것 — 짧은 소리 셋 (2026-09-02 확인)

Suno는 **곡을 만드는 모델**이라 1~10초짜리는 안 나온다. 실제로 시험해 봤다:
"2초 간격으로 한 방씩 띄엄띄엄"이라고 지시한 소리 창고용 트랙을 뽑았더니
45초 내내 이어지는 연주곡이 나왔다(어택 129개, 초당 3회꼴). 지시를 못 따른다.

그래서 아래 셋은 **Suno에서 제외**하고 따로 만든다.

| 트랙 | 길이 | 처리 |
|---|---|---|
| `sfx_fanfare` | 2~3초 | 합성(Karplus-Strong 뜯은 현 + 비조화 배음 징) |
| `bgm_intro_sting` | 7초 | 합성(징 한 방 + 여운) |
| `bgm_chapter_end` | 9초 | 합성(내려가는 뜯음 + 징) |

합성 스크립트는 scratchpad의 `synth.py`. 음정·길이·밝기가 전부 숫자라 바로 고쳐 뽑을 수 있다.
사실적인 실제 녹음이 필요하면 ElevenLabs Sound Effects(효과음 전용, 유료 플랜에 상업 사용 포함)나
Freesound.org(CC0만 쓸 것 — CC-BY는 앱 안 출처 표기 필요)가 대안이다.
