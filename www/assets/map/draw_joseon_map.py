...
def main():
    ...
    points = {
        ...
        #
        # goryeo(고려 대) — 이 지도는 원래 '조선 8도' 지도라 early/late뿐이었는데,
        # 고려 챕터(강동 6주·동북 9성)가 이 지도를 그대로 빌려 쓰면서 993~1109년
        # 장면에 '명(明)'이 뜨는 시대착오가 났다(성일님 제보: "명, 여진이 나오네.
        # 거란은 어디있어?" — 명은 1368년 건국이라 375년을 앞당긴 꼴이었다).
        # 명 자리에 거란(요)을 놓는다 — 두 사건 모두 요동 쪽 그 이웃과의 일이라
        # 좌표는 그대로 쓴다. 여진은 고려 대에도 실제 이웃이었으므로 그대로 둔다.
        'countries': [
            {'id': 'ming',    'lonlat': [123.95, 40.95], 'early': '명(明)',     'late': None,       'goryeo': '거란(遼)'},
            {'id': 'jurchen', 'lonlat': [129.30, 43.35], 'early': '여진(女眞)', 'late': None,       'goryeo': '여진(女眞)'},
            {'id': 'qing',    'lonlat': [126.60, 42.95], 'early': None,         'late': '청(淸)',   'goryeo': None},
            {'id': 'japan',   'lonlat': [130.70, 33.35], 'early': '일본(日本)', 'late': '일본(日本)', 'goryeo': '일본(日本)'},
        ],
        ...
        'markers': [
            ...
            # 강동 6주 — 서희가 993년 소손녕과 담판해 확보한 여섯 고을. 모두
            # 평안북도 청천강~압록강 사이에 있다(귀주=구성, 곽주=정주 곽산,
            # 철주=철산, 용주=용천, 통주=선천, 흥화진=의주 인근). 여섯 점을
            # 따로 찍는다 — loff는 라벨이 서로 안 겹치게 미리 흩어 놓은 자리
            # (픽셀 오프셋, 프로젝션 좌표계 기준).
            {'id': 'heunghwajin', 'lonlat': [124.45, 40.14], 'label': '흥화진', 'loff': [0, -15]},
            {'id': 'gwiju',       'lonlat': [125.18, 39.98], 'label': '귀주',   'loff': [10, -15]},
            {'id': 'tongju',      'lonlat': [124.85, 39.90], 'label': '통주',   'loff': [0, -15]},
            {'id': 'yongju',      'lonlat': [124.60, 39.85], 'label': '용주',   'loff': [-22, 4]},
            {'id': 'cheolju',     'lonlat': [124.68, 39.72], 'label': '철주',   'loff': [0, 16]},
            {'id': 'gwakju',      'lonlat': [125.05, 39.72], 'label': '곽주',   'loff': [10, 16]},
            # 동북 9성 — 윤관이 1107년 별무반으로 여진을 몰아내고 쌓은 아홉 성.
            # 정확한 위치·범위는 학계에서도 논쟁 중이라(일부는 두만강 이북까지
            # 봤다고 봄) 교과서가 따르는 통설인 '함흥평야 일대' 안에서, 통용되는
            # 배치도(9개 성 상대 위치)를 참고해 아홉 점을 흩어 찍는다.
            {'id': 'jinyangjin',  'lonlat': [127.30, 40.12], 'label': '진양진', 'loff': [-10, -16]},
            {'id': 'yeongju9',    'lonlat': [127.55, 40.20], 'label': '영주',   'loff': [0, -16]},
            {'id': 'gonghyeomjin','lonlat': [127.80, 40.12], 'label': '공험진', 'loff': [14, -16]},
            {'id': 'gilju',       'lonlat': [128.05, 40.02], 'label': '길주',   'loff': [16, -4]},
