...
window.Items = (function(){
  ...
  const DB = {
    ...
    yugilo:       { name:'6·15 남북공동선언', era:'현대', emoji:'🕊️',
      ...
      exam:'2000년 **김대중** 대통령과 김정일 국방위원장의 첫 남북 정상회담에서 발표됐다.' },    /* 2026-09-17 교안 빈자리용 16점 */
    sehyeongdonggeom: { name:'세형 동검', era:'철기', emoji:'🗡️',
      desc:'좁고 곧게 뻗은 청동검. 검 몸 가운데 등날이 선다. 한국식 동검이라고도 한다.',
      exam:'**철기 시대** 한반도의 **독자적 청동기 문화**. 거푸집과 함께 나와 이 땅에서 만들었음을 보여 준다.' },
    dahori_but:   { name:'다호리 붓', era:'철기 · 삼한', emoji:'🖌️',
      desc:'옻칠한 나무 붓대의 양 끝에 붓털을 단 붓. 글자를 깎아 지우던 손칼과 함께 나왔다.',
      exam:'**창원 다호리** 1호 널무덤 출토. 기원전 1세기 무렵 **한자를 썼다**는 증거.' },
    houmyeong:    { name:'호우명 그릇', era:'삼국 · 신라', emoji:'🥣',
      desc:'뚜껑 덮인 청동 그릇. 바닥에 광개토대왕을 기리는 글자가 새겨졌다.',
      exam:'경주 **호우총** 출토. 고구려 그릇이 신라 무덤에서 나와 **5세기 고구려와 신라의 관계**를 보여 준다.' },
    yeonga7:      { name:'연가 7년명 금동 여래 입상', era:'삼국 · 고구려', emoji:'🛕',
      desc:'불꽃무늬 광배를 진 작은 금동 불상. 뒷면에 글자가 새겨졌다.',
      exam:'**고구려** 불상인데 **경남 의령**에서 나왔다. 두꺼운 옷과 길쭉한 얼굴이 특징.' },
    imsinseogi:   { name:'임신서기석', era:'삼국 · 신라', emoji:'🪨',
      desc:'냇돌에 두 젊은이가 맹세한 글을 새긴 돌.',
      exam:'신라 청년이 **유교 경전**을 익히겠다고 맹세한 기록 — 신라의 **유학 교육**을 보여 준다.' },
    jeokseongbi:  { name:'단양 신라 적성비', era:'삼국 · 신라', emoji:'🪨',
      desc:'윗부분이 깨진 화강암 비석. 앞면에 글자가 빽빽하다.',
      exam:'**진흥왕**이 남한강 상류 **적성**을 차지한 뒤 세웠다. 한강 유역 진출의 증거.' },
    sangwonsajong: { name:'상원사 동종', era:'남북국 · 통일신라', emoji:'🔔',
      desc:'용 모양 고리와 음통을 단 청동 범종. 몸통에 악기를 연주하는 비천상이 새겨졌다.',
      exam:'725년에 만든 **현존 가장 오래된 한국 범종**. 평창 오대산 상원사에 있다.' },
    balhae_seokdeung: { name:'발해 석등', era:'남북국 · 발해', emoji:'🏮',
      desc:'현무암으로 만든 6미터 높이의 석등. 연꽃잎을 크게 새긴 팔각 받침을 가졌다.',
      exam:'**상경성** 절터에 남은 발해 유물. 힘찬 연꽃무늬에서 **고구려의 기상**을 읽는다.' },
    balhae_saja:  { name:'발해 돌사자상', era:'남북국 · 발해', emoji:'🦁',
      desc:'고개를 들고 앉은 작은 돌사자.',
      exam:'**정혜공주 묘**에서 나왔다. 굳센 모습이 고구려 조각을 닮았다고 본다.' },
    yeonggwangtap: { name:'발해 영광탑', era:'남북국 · 발해', emoji:'🧱',
      desc:'벽돌로 쌓은 다섯 층의 네모난 탑. 높이는 약 13미터다.',
      exam:'지금 중국 지린성 장백현에 남은 **발해의 전탑**. 탑 아래에 무덤 칸이 있다.' },
    suncheongja:  { name:'순청자(청자 참외모양 병)', era:'고려 중기', emoji:'🏺',
      desc:'무늬 없이 비색만으로 멋을 낸 청자 병. 참외 모양 몸통에 긴 목이 올라간다.',
      exam:'상감 청자 이전 **12세기 순청자**의 대표작. 인종의 무덤 장릉에서 나왔다고 전한다.' },
    gonyeo:       { name:'곤여만국전도', era:'조선 후기', emoji:'🗺️',
      desc:'타원형 안에 다섯 대륙을 그린 세계지도.',
      exam:'마테오 리치가 만든 서양식 세계지도. 조선에 들어와 **세계관이 넓어지는** 계기가 됐다.' },
    chochungdo:   { name:'초충도', era:'조선 중기', emoji:'🦗',
      desc:'오이 넝쿨과 방아깨비, 나비를 그린 그림.',
      exam:'**신사임당**이 그렸다고 전하는 그림. 풀과 벌레를 섬세하게 그렸다.' },
    sokdaejeon:   { name:'속대전', era:'조선 후기', emoji:'📘',
      desc:'푸른 표지를 붉은 실로 묶은 법전.',
      exam:'**영조** 때 경국대전 이후의 법령을 모아 편찬했다.' },
    daejeontongpyeon: { name:'대전통편', era:'조선 후기', emoji:'📚',
      desc:'두툼한 누런 표지의 법전 두 권.',
      exam:'**정조** 때 경국대전·속대전을 합쳐 편찬한 통합 법전.' },
    hwangseong:   { name:'황성신문', era:'대한제국', emoji:'📰',
      desc:'세로로 빽빽한 글자 단이 이어진 옛 신문. 한가운데 논설 칸이 있다.',
      exam:'1905년 11월 20일 **「시일야방성대곡」**을 실어 **을사늑약**의 부당함을 알렸다.' },
  };
